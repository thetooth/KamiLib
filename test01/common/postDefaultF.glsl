...
vec4 gaussianblur(bool procedural)
{
	...
	for (float x = -KERNEL_SIZE + 1.0; x < KERNEL_SIZE; x += 1.0 )
	{
		...
		{
			blurTex += bright( texcoord + vec2( blur.x * x * (rand(gl_FragCoord.xy*shaderClock)/2.0+1.0), blur.y * y * (rand(gl_FragCoord.xy*shaderClock)/2.0+1.0)), procedural);
		}
...
{
	// Plain
	vec3 stage0 = (texcolor.rgb)*((rand(gl_FragCoord.xy*shaderClock)/9.5)+1.0);
	//vec3 stage1 = gaussianblur(false).rgb;
