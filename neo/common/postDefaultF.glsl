...
vec4 gaussianblur(bool procedural)
{
	...
	for (float x = -KERNEL_SIZE + 1.0; x < KERNEL_SIZE; x += 1.0 )
	{
		...
		{
			blurTex += bright( texcoord + vec2( blur.x * x * (rand(gl_FragCoord.xy*time)/2.0+1.0), blur.y * y * (rand(gl_FragCoord.xy*time)/2.0+1.0)), procedural);
		}

	}
	blurTex /= ((KERNEL_SIZE+KERNEL_SIZE)-1.0)*((KERNEL_SIZE+KERNEL_SIZE)-1.0);
	return blurTex;
}
...
float vignette(float bounds = 1.0, float offset = 0.0, vec2 pos = vec2(0.5, 0.5)){
	return smoothstep(bounds, offset, distance(texcoord, pos));
}

vec3 crosshatch(vec3 rgboffset, int multiplyer = 2, int comp = 0){
	float val = 0.0;
	if(int(gl_FragCoord.y)%multiplyer == comp && int(gl_FragCoord.x)%multiplyer == comp){
		val = 1.0;
	}else if(int(gl_FragCoord.y)%multiplyer != comp && int(gl_FragCoord.x)%multiplyer != comp){
		val = 1.0;
	}
	return vec3(val*rgboffset.x, val*rgboffset.y, val*rgboffset.z);
}


void main(){
	float grain = (rand(gl_FragCoord.xy)/20.0)+1.0;
	vec3 stage0 = (gradient(texcolor).rgb)*grain;//+gaussianblur(true).rgb);//*grain;
	//vec3 stage1 = crosshatch(vec3(0.5, 0.5, 0.5), 2, 0);
	//vec3 stage2 = BlendOverlay(stage0, stage1);
