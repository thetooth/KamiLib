...
Snake::Snake(): direction_(static_cast<Direction>(rand() % 4)), lastMove_(direction_)
{
	blocks_.push_back(std::pair<int, int>(Field::WIDTH / 2, Field::HEIGHT / 2));
	for (int i = 0; i < Field::WIDTH/4; i++)
	{
		blocks_.push_back(std::pair<int, int>(0, 0));
	}
}
